...
# ==========================================
# 4. AI SERVICE LAYER (V11.8 - SMART SCALING)
# ==========================================
def process_image_with_gemini(image, api_key):
    ...
    prompt = """
    Bạn là một trợ lý tài chính (OCR). Nhiệm vụ: Trích xuất dữ liệu từ ảnh.
    
    Hãy tìm các trường sau (nếu có):
    1. Mã chứng khoán (Symbol): Ưu tiên tìm mã CW. Nếu là Bảng giá (chỉ có VHM), lấy mã đó.
    2. Số lượng (Qty): Khối lượng mua/nắm giữ.
    3. Giá vốn (Price): Giá khớp lệnh, giá trung bình. (Bảng giá KHÔNG có giá này -> null).
...
def main():
    ...
    st.caption(f"System: V11.8 | Build: {build_time_str} | Smart Scaling (x1000)")
    ...
    with st.sidebar:
        ...
        if uploaded_img and api_key:
            if st.button("🚀 Phân tích ngay"):
                with st.spinner("Đang xử lý (Auto x1000 nếu cần)..."):
                    ...
                    if "error" in result: st.error(result['error'])
                        ...
                        # --- LOGIC THÔNG MINH: SMART SCALING ---
                        # Nếu giá AI đọc được < 1000 -> Tự động nhân 1000
                        
                        if result.get('market_price'): 
                            raw_mp = float(result['market_price'])
                            if raw_mp < 1000 and raw_mp > 0: # Ngưỡng an toàn
                                raw_mp = raw_mp * 1000
                                msg += " (Đã x1000)"
                            st.session_state['temp_ocr_market_price'] = raw_mp
                            msg += f" | Giá TT: {raw_mp:,.0f}"
                            
                        if result.get('price'): 
                            raw_p = float(result['price'])
                            if raw_p < 1000 and raw_p > 0: # Áp dụng cho cả giá vốn nếu cần
                                raw_p = raw_p * 1000
